...
✅ DONE 2026-07-06, Fable-verified — `_process_native_faces()` in engine.py: detection + person segmentation stay at proxy; face bboxes + ied scale to native (`bbox * 1/proxy_scale`, `ied * 1/proxy_scale`); MediaPipe landmarks are normalized [0,1] so resolution-independent (no scaling needed); person_mask upscaled INTER_LINEAR to native; reshape + per-face parsing (BiSeNet on native crop — sharper region boundaries than the plan's "upscale per-face" suggestion, same 512×512 ONNX inference cost) + `_process_face_core` + composite all run at native. Global phases already native (F8.1). F8.1 `_composite_upscaled_faces_onto_native` + F8.0 reinjection bypassed (no proxy-res face result to upscale/reinject — faces never went through proxy). `quality="draft"` ctx flag (`ProcessingContext.quality`, default `"full"`) keeps the legacy F8.1 all-proxy path for batch contact sheets. **Per-face op audit:** `local_clarity` had a hardcoded `radius=20` that didn't scale with resolution — fixed to `max(8, int(face_width * 0.04))` (perf_optimizations.py:601); all other ops already scale via `face_width`/`ied` (which is now native-res). `guided_filter`'s `max_dim=1200` is a compute-downsample (coefficients at 1200, applied at full res), not a fidelity cap. `face_contexts` cache is invalidated when `proxy_scale < 1.0` in F8.2 path (proxy-res regions can't be reused at native). Tests: 112 engine/F8 targeted tests pass, 2 updated proxy-pipeline tests pass (assert F8.2 default doesn't call `_upscale_core_result`, `quality="draft"` does). `test_detects_face` failure is pre-existing (unrelated, no-face-detected on real test image). **Receipt:** engine.py (`_process_native_faces`, `quality` field, `process(quality=)` kwarg), perf_optimizations.py (local_clarity radius scaling), tests/test_integration.py (updated proxy tests).
