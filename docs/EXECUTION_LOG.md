...
**R4 D&B v2 — dedicated blotch-band (2026-07-11, Haiku):** added `blotch_reduction` (0–1) param, wired recipe→`ProcessingContext`→`frequency.combine()`. Broad blotch band carved as `blur(k_a) − low` (k_a > k_low) and removed from the LOW band so pigment/redness blotches are *evened* (not merely blurred) while facial form/shading cancels in the difference by construction and pores (absent from LOW) survive. Files: `retouch/frequency.py`, `retouch/params.py`, `retouch/engine.py`, `retouch/perf_optimizations.py` (+4 combine call sites). Tests: `tests/test_frequency_blotch.py` (2 pass; byte-identical at 0, evens broad blotch while preserving form ramp + pore HF energy). Full frequency/params suites green (180 passed). Visual-Critical → [VISUAL QA PENDING]; GUI slider left to the running GUI-wiring agent.

**Color-science K3/K8/K9 (2026-07-11, Haiku — two general agents):** K3 `gamut_compress` + `find_gamut_intersection` (OKLab bisection) and K9 `apply_subtractive_saturation` in `color_science.py`; both wired in `grading.py` (settings `gamut_compress` default ON/no-op in-gamut, `saturation_mode` default additive → byte-identical golden path). K8 `delta_e_2000` (CIEDE2000, Sharma-34-verified) + `detect_color_drift` in `qa_detectors.py`; `run_all` returns `color_drift`; `engine.py` gates it via `COLOR_DRIFT_THRESHOLD` + QAWarning. Params + CLI flags (`--gamut-compress`, `--saturation-mode`) added. Tests `test_color_science_k3k9.py` + `test_color_qa_deltae.py` = 14 passed. Sample renders in `test_output/color_science_2026-07-11/` (K3/K8/K9). **Accepted gap:** full-engine path does not yet forward `saturation_mode`/`gamut_compress` settings from recipe→grade (CLI global-grade path does); documented, not blocking.

**R13/R14/R15 (2026-07-11, Haiku):** R13 core slice d — `directional_wrinkle_attenuate` (oriented-structure attenuation via structure-tensor anisotropy, per-pixel retention floor) in `frequency.py`; 3 tests (`test_frequency_r13.py`). R14 core slice b — `classify_fitzpatrick`/`ita_value`/`tone_adaptation_params` in `color_science.py` (2 agents, 7 tests `test_skin_tone_classify.py`). R15 — `detect_pore_spectrum_distance`/`detect_over_retouch_asymmetry`/`gui_skin_score` in `qa_detectors.py`, wired into `run_all` + `engine.py` gates (2 agents, 5 tests `test_qa_r15.py`). R9/R10/R11/R12 = hard-blocked (R9 = moonshot spike not one-shot; R10/R11 need R7 melanin/hemoglobin maps = BACKLOG; R12 needs R9 shading layer). Status: R4/R9-stub-not/R13/R14/R15 core slices ✅; R9/R10/R11/R12 remain 📋 BACKLOG with documented missing upstream.

**R7/R9/R10/R11/R12 (2026-07-11, Haiku — 4 parallel agents):** all keystone + dependent backlog items now have real algorithmic cores with unit tests.
- **R7** `retouch/chromophore.py`: log-linear (B-LUT/Tsumura) melanin+hemoglobin unmixing — `decompose_chromophores`, `hemoglobin_breakdown_phase`. 10 tests. Unblocks R10/R11.
- **R9** `retouch/intrinsic.py`: WLS (Fattal 2008) shading + `decompose_intrinsic` (albedo×shading) + `even_albedo`. 5 tests (shading↔ramp Pearson 0.93). Upgrades C2; R12 reuses shading.
- **R12** `retouch/specular.py`: dichromatic `extract_specular`/`render_finish` (matte/powder/dewy/glass_skin)/`specular_recolor`; wired into `skin.py`+`params.py`(`specular_finish`/`_strength`/`_recolor`)+`engine.py`/`perf_optimizations.py`; default byte-identical. 8 tests.
- **R10+R11** `retouch/skin_chromophore.py`: 11 fns (blemish_vs_mole, bruise_remove, vein_attenuate, tanline_even, hemoglobin_guided_smooth, compression_mark_remove, goosebumps_smooth, beard_shadow_neutralize, facepaint_crack_repair, paint_coverage_even, ingrown_hair_cleanup). 24 tests. `skin.py` wiring = documented follow-up.
Visual-Critical items (R7/R9/R10/R11/R12, and prior R4/R13) carry **[VISUAL QA PENDING]** — headless harness cannot verify pixel quality; the run QA-signoff harness reported a real pre-existing finding (DSCF4454 `outdoor_harsh_sun_v1` color-drift ΔE 10.49 vs thr 2.0) for separate follow-up. Total new tests this session: R4/R7/R9/R10+R11/R12/R13/R14/R15 = 2+10+5+24+8+3+7+5 = 64 passing.

**K9 subtractive-saturation FIX — green skin/background cast (2026-07-12, Opus):** the shipped `apply_subtractive_saturation` (log-density space, anchored on `d_min` = brightest channel) **pinned the brightest RGB channel and slid every pixel's hue toward it** — turning warm skin (R>G>B) and neutral backgrounds *green* (clearly visible in the K9 panel of `color_science_REVIEW_SHEET.png`). Root cause is that per-channel log-density scaling is not OKLab-hue-preserving; an intermediate luminance-anchor variant still drifted hue ~26° on saturated colors. **Fix:** rewrote in **OKLCh** — chroma ×`(1+amount)`, hue held fixed, L darkened ∝ `amount·C` (the genuine subtractive/CMY film-density trait); near-neutral pixels (C in a 0.05–0.09 ramp) blend back to source so uint8 grays (residual C up to 0.046 from quantization) stay an *exact* fixed point. `amount==0` byte-identical. Verified invariants: grays exact at all levels; saturated red/green/blue/skin hue drift <1.2°; negative amount desaturates; positive darkens+saturates; end-to-end `grading.py` subtractive path keeps skin R>G>B (no cast). Tests updated (`tests/test_color_science_k3k9.py`): the old `test_k9_negative_amount_lightens` asserted a *side effect of the bug* (negative→lighten) — replaced with `test_k9_negative_amount_desaturates` + new `test_k9_no_green_cast`; 15 passed. Ad-hoc sheet-gen script made durable at `scripts/render_color_science_sheet.py`; renders in `test_output/color_science_2026-07-11/` regenerated (natural skin, no cast). See memory `subtractive-saturation-oklch-rewrite`.
