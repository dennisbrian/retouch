...
def _retinex_normalize(chan):
    low = float(np.percentile(chan, 1))
    high = float(np.percentile(chan, 99))
    if high - low < 1e-5:
        return np.zeros_like(chan)
    return np.clip((chan - low) / (high - low) * 255.0, 0.0, 255.0)


def retinex_msr(image_bgr, skin_mask=None, sigmas=(15, 80, 250),
                gain=1.0, offset=0.0):
    """Multi-Scale Retinex (MSR).

    Computes log(I) - log(I*Gaussian) at multiple scales and averages.
    Optionally restricted to skin_mask region.

    Returns retinex-corrected BGR image.
    """
    img = image_bgr.astype(np.float32)
    retinex_sum = np.zeros_like(img, dtype=np.float32)
    for sigma in sigmas:
        blurred = cv2.GaussianBlur(img, (0, 0), float(sigma))
        retinex_sum += np.log(img + 1.0) - np.log(blurred + 1.0)
    retinex = retinex_sum / float(len(sigmas))
    retinex = retinex * gain + offset

    out = np.zeros_like(retinex, dtype=np.float32)
    for c in range(retinex.shape[2]):
        out[:, :, c] = _retinex_normalize(retinex[:, :, c])
...
def retinex_ssr(image_bgr, sigma=80, skin_mask=None):
    """Single-Scale Retinex."""
    return retinex_msr(image_bgr, skin_mask=skin_mask, sigmas=(float(sigma),))
