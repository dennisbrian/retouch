...
### Known Limitations
- Detection on real corpora (2026-08-19 study, `docs/plans/RESEARCH_DETECTION_RECALL_2026_08_19.md`): subject-face miss rate measured then fixed same-day — legacy path now dual-scale (2048+1024 person-gated union, 100% subject recall on the 83-image DSCF corpus, +22 ms median). Background/crowd-face recall 27–35% (out of scope for portrait retouch). **Open:** MediaPipe fires on anime posters/banners (18 pre-existing FP detections on the same corpus) which receive per-face work at strong recipes — precision, not recall, is the open convention-corpus problem. RetinaFace is NOT in requirements: the `cc61e67` F1/F2 fixes are inert in the pinned runtime; production detection is MediaPipe-only.
